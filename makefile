...
TARGET_FILE = slides-math.cls
TARGET_DIR = slides-math

REMOTE = adarienzo@osmium.us.to
REMOTE_DIR = /home/adarienzo/.local/share/texmf/tex/latex/slides-math/slides-math.cls

local-install:
	$(info Updating local $(TARGET_FILE))
	cp ./$(TARGET_FILE) "$(TEXFOLDER)/$(TARGET_DIR)/$(TARGET_FILE)"
